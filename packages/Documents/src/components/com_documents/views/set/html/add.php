...
<script src="com_documents/js/organizer.js" />
<? else: ?>
<script src="com_documents/js/min/organizer.min.js" />
<? endif; ?>
...
<div class="row">
    ...
    <form id="set-form" method="post" action="<?= @route('view=set&oid='.$actor->id) ?>">
    	...
    	<fieldset>
    		...
    		<div id="document-selector"></div>

    		<div id="set-documents" class="an-entities">
...
<script>
<? $url = 'option=com_documents&view=documents&layout=selector&oid='.$actor->id; ?>
$('#document-selector').setOrganizer( 'open', '<?= @route($url) ?>');
</script>
