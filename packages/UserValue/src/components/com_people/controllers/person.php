...
class ComPeopleControllerPerson extends ComActorsControllerDefault
{
    ...
    protected function _initialize(KConfig $config)
    {
		...
		$this->_allowed_academic_types = array(
			...
			ComPeopleDomainEntityPerson::ACADEMICTYPE_TEACHER,
        );
		...
		$this->_allowed_corporate_types = array(
			...
			ComPeopleDomainEntityPerson::CORPORATETYPE_EMPLOYER,
        );
		...
		/*
		if ($viewer->Teacher()) {
			$this->_allowed_academic_types[] = ComPeopleDomainEntityPerson::ACADEMICTYPE_TUTOR;
		}
		if ($viewer->Employer()){
			$this->_allowed_corporate_types[] = ComPeopleDomainEntityPerson::CORPORATETYPE_RECRUITER;
		}
		*/
    ...
    protected function _actionBrowse(KCommandContext $context)
    {
		...
		/* Copied from above to add Sparq Functionality --- William */
		if ($this->filter) {
            if ($this->filter['academictype'] && in_array($this->filter['academictype'], $this->_allowed_academic_types)) {
                $query->filterUsertype($this->getService('koowa:filter.cmd')
                      ->sanitize($this->filter['academictype']));
            }
		...
		if ($this->filter) {
            if ($this->filter['corporatetype'] && in_array($this->filter['corporatetype'], $this->_allowed_corporate_types)) {
                $query->filterUsertype($this->getService('koowa:filter.cmd')
                      ->sanitize($this->filter['corporatetype']));
            }
