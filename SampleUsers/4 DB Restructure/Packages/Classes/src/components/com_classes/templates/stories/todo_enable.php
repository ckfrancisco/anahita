...
<? else : ?>
<data name="title">
	<?= sprintf(@text('COM-CLASSES-STORY-CLASSES-OPENED'), @name($subject), @route($object->getURL()))?>
</data>
<data name="body">
	<?
        $priority = '';
    ...
    ?>
	<div>
		<?= @link($object) ?> <span class="an-meta"> - (<?= @text('COM-CLASSES-CLASSES-PRIORITY') ?>: <?=$priority?>)</span>
	</div>
</data>
<? endif; ?>
