...
<? if (is_array($object)) : ?>
<data name="title">
	<?= sprintf(@text('COM-TESTER-STORY-TESTER-OPENED'), @name($subject)) ?>
</data>
<data name="body">
<ol>
...
<? else : ?>
<data name="title">
	<?= sprintf(@text('COM-TESTER-STORY-TESTER-OPENED'), @name($subject), @route($object->getURL()))?>
</data>
<data name="body">
	<?
        $priority = '';
    ...
    ?>
	<div>
		<?= @link($object) ?> <span class="an-meta"> - (<?= @text('COM-TESTER-TESTER-PRIORITY') ?>: <?=$priority?>)</span>
	</div>
</data>
<? endif; ?>
