...
class ComClassesDomainEntityComponent extends ComMediumDomainEntityComponent
{
    ...
    {
        $config->append(array(
            'story_aggregation' => array('class_disable,class_add,class_enable' => 'target'),
            'behaviors' => array(
                    'scopeable' => array('class' => 'ComClassesDomainEntityClass'),
                    'hashtagable' => array('class' => 'ComClassesDomainEntityClass'),
                ),
        ));
    ...
    {
        if ($actor->authorize('action', 'com_classes:class:add')) {
            $composers->insert('classes-composer', array(
                'title' => AnTranslator::_('COM-CLASSES-COMPOSER-CLASS'),
                'placeholder' => AnTranslator::_('COM-CLASSES-CLASS-ADD'),
                'url' => 'option=com_classes&view=class&layout=composer&oid='.$actor->id,
            ));
        }
    }
